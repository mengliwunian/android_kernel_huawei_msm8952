				...
				int irq)
{
	...
	ad_dev_logn(codec->dev, "%s: clear ocp status %x\n",
...
static void wcd_program_hs_vref(struct wcd_mbhc *mbhc)
{
	...
	ad_dev_logd(codec->dev, "%s: reg_val  = %x\n", __func__, reg_val);
					 ...
					 struct work_struct *work)
{
	...
	if (cancel_work_sync(work)) {
		ad_logd("%s: correct_plug_swch is canceled\n",
			 __func__);
		mbhc->mbhc_cb->lock_sleep(mbhc, false);
	}
...
static void wcd_mbhc_clr_and_turnon_hph_padac(struct wcd_mbhc *mbhc)
{
	...
	if (test_and_clear_bit(WCD_MBHC_HPHL_PA_OFF_ACK,
			       &mbhc->hph_pa_dac_state)) {
		ad_logd("%s: HPHL clear flag and enable PA\n", __func__);
	...
	if (pa_turned_on) {
		ad_logn("%s: PA was turned on by MBHC and not by DAPM\n",
			 __func__);
...
static bool wcd_is_special_headset(struct wcd_mbhc *mbhc)
{
	...
	if (mbhc->mbhc_cb->mbhc_micbias_control &&
	    !mbhc->mbhc_cb->mbhc_micb_ctrl_thr_mic) {
		ad_logd("%s: callback fn micb_ctrl_thr_mic not defined\n",
			 __func__);
		return false;
		...
		rc = mbhc->mbhc_cb->mbhc_micb_ctrl_thr_mic(codec,
							MIC_BIAS_2, true);
		if (rc) {
			ad_loge("%s: Micbias control for thr mic failed, rc: %d\n",
				__func__, rc);
			return false;
		}
	}
...
static void wcd_correct_swch_plug(struct work_struct *work)
{
	...
	if (!rc) {
		ad_logd("%s No btn press interrupt\n", __func__);
...
correct_plug_type:
	...
	while (!time_after(jiffies, timeout)) {
		...
		if (hs_comp_res && !(hphl_sch || mic_sch)) {
			ad_logd("%s: cable is extension cable\n", __func__);
			plug_type = MBHC_PLUG_TYPE_HIGH_HPH;
			wrk_complete = true;
		} else {
...
static int wcd_mbhc_get_button_mask(struct wcd_mbhc *mbhc)
{
	...
	case 5:
		mask = SND_JACK_BTN_5;
		ad_logn("%s: detect SND_JACK_BTN_5\n", __func__);
		break;
	case 6:
		mask = SND_JACK_BTN_6;
		ad_logn("%s: detect SND_JACK_BTN_6\n", __func__);
		break;
	case 7:
		mask = SND_JACK_BTN_7;
		ad_logn("%s: detect SND_JACK_BTN_7\n", __func__);
		break;
	default:
		break;
	}
...
static irqreturn_t wcd_mbhc_hs_ins_irq(int irq, void *data)
{
	...
	ad_logd("%s: detection_type %d, elect_result %x\n", __func__,
...
static irqreturn_t wcd_mbhc_hs_rem_irq(int irq, void *data)
{
	...
	do {
		...
		ad_logd("%s: Check result reg for fake removal: hs_comp_res %x\n",
...
static void wcd_btn_lpress_fn(struct work_struct *work)
{
	...
	WCD_MBHC_REG_READ(WCD_MBHC_BTN_RESULT, btn_result);
	if (mbhc->current_plug == MBHC_PLUG_TYPE_HEADSET) {
		ad_logd("%s: Reporting long button press event, btn_result: %d\n",
			 __func__, btn_result);
...
static irqreturn_t wcd_mbhc_release_handler(int irq, void *data)
{
	...
	if (mbhc->buttons_pressed & WCD_MBHC_JACK_BUTTON_MASK) {
		...
		if (ret == 0) {
			ad_logn("%s: Reporting long button release event\n",
				 __func__);
			wcd_mbhc_jack_report(mbhc, &mbhc->button_jack,
...
static irqreturn_t wcd_mbhc_hphr_ocp_irq(int irq, void *data)
{
	...
	ad_logd("%s: received HPHR OCP irq\n", __func__);
...
#ifdef CONFIG_HUAWEI_KERNEL
bool huawei_audio_is_factory_mode(void)
{
    static enum runmode_enum run_mode = RUNMODE_INIT;

    if(RUNMODE_INIT == run_mode)
    {
        ...
        if(saved_command_line != NULL)
        {
            ...
            {
                run_mode = RUNMODE_FACTORY;
            }
        }
        ad_logn("%s run mode is %d\n", __func__, run_mode);
    }

    if(RUNMODE_FACTORY == run_mode)
    {
        return true;
    }
    else
...
static int wcd_mbhc_initialise(struct wcd_mbhc *mbhc)
{
	...
	wcd_program_btn_threshold(mbhc, false);
#ifdef CONFIG_HUAWEI_KERNEL
	is_factory = huawei_audio_is_factory_mode();
	ad_logn("%s: is_factory_mode = %d\n", __func__, is_factory);
#endif
	...
	for (i = 0 ; i < WCD_MBHC_KEYCODE_NUM ; i++) {
		if (btn_key_code[i] != 0) {
			...
			ret = snd_jack_set_key(mbhc->button_jack.jack,
							...
							btn_key_code[i]);
			if (ret) {
				ad_loge("%s: Failed to set code for %d\n",
			...
			ad_logd("%s: set btn%d key code:%d\n", __func__,
				i, btn_key_code[i]);
		}
	}
	if (btn_key_code[0])
		       ...
		       struct wcd_mbhc_config *mbhc_cfg)
{
	...
	if ((!mbhc->is_btn_already_regd) && wcd_mbhc_set_keycode(mbhc))
		ad_loge("Set btn key code error!!!\n");
		      ...
		      bool impedance_det_en)
{
	...
	if (!mbhc->wcd_mbhc_regs) {
		ad_dev_loge(codec->dev, "%s: mbhc registers are not defined\n",
			__func__);
		return -EINVAL;
	}
	...
	if (!mbhc_cb || !mbhc_cb->request_irq || !mbhc_cb->irq_control ||
	    ...
	    !mbhc_cb->set_btn_thr) {
		ad_dev_loge(codec->dev, "%s: required mbhc callbacks are not defined\n",
			__func__);
		return -EINVAL;
	}
