...
static int enable_spk_ext_pa(struct snd_soc_codec *codec, int enable)
{
	...
	if (enable) {
		...
		if (ret) {
			ad_loge("%s: gpio set cannot be de-activated %s\n",
	...
	if (pdata->us_euro_gpio < 0) {
		...
		if (!gpio_is_valid(pdata->us_euro_gpio)) {
			...
			ret = msm_get_gpioset_index(CLIENT_WCD_INT,
							"us_eu_gpio");
			if (ret < 0) {
				ad_loge("%s: gpio set name does not exist: %s",
							__func__, "us_eu_gpio");
				return ret;
			}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	case 1:
		...
		if (ret) {
			ad_loge("%s: failed to enable the pri gpios: %d\n",
					__func__, ret);
			break;
		}
		mutex_lock(&pdata->cdc_mclk_mutex);
		if ((!atomic_read(&pdata->mclk_rsc_ref)) &&
			...
			if (ret < 0) {
				...
				ret = msm_gpioset_suspend(CLIENT_WCD_INT,
								"pri_i2s");
				if (ret)
					ad_loge("%s: failed to disable the pri gpios: %d\n",
							__func__, ret);
				break;
			}
			atomic_set(&pdata->mclk_enabled, true);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	ad_logd("%s: msm_vi_feed_tx_ch = %ld\n", __func__,
				ucontrol->value.integer.value[0]);
	return 0;
}
			      ...
			      struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		ret = msm_gpioset_suspend(CLIENT_WCD_INT, "pri_i2s");
		if (ret < 0) {
			ad_loge("%s: gpio set cannot be de-activated %sd",
					__func__, "pri_i2s");
			return ret;
		}
		if (atomic_read(&pdata->mclk_rsc_ref) == 0) {
			      ...
			      struct snd_kcontrol *kcontrol, int event)
{
	...
	if (!supply->supply) {
		ad_dev_loge(w->codec->card->dev, "%s: no wsa switch supply",
			__func__);
		return ret;
	}
	...
	if (enable) {
		if (!atomic_read(&pdata->wsa_mclk_rsc_ref)) {
			...
			ret = afe_set_lpass_clock(
					...
					&wsa_ana_clk);
			if (ret < 0) {
				ad_loge("%s: failed to enable mclk %d\n",
					__func__, ret);
				goto done;
			}
		}
		...
		if (!atomic_dec_return(&pdata->wsa_mclk_rsc_ref)) {
			...
			ret = afe_set_lpass_clock(
					...
					&wsa_ana_clk);
			if (ret < 0) {
				ad_loge("%s: failed to disable mclk %d\n",
					__func__, ret);
				goto done;
			}
		}
...
static int msm_mi2s_snd_startup(struct snd_pcm_substream *substream)
{
	...
	if (card->aux_dev && substream->stream ==
		...
		if (ret < 0) {
			ad_loge("%s: failed to enable mclk for wsa %d\n",
				__func__, ret);
			return ret;
		}
	}
	...
	ret = msm_gpioset_activate(CLIENT_WCD_INT, "pri_i2s");
	if (ret < 0) {
		ad_loge("%s: gpio set cannot be activated %sd",
				__func__, "pri_i2s");
		return ret;
	}
	msm8x16_wcd_mclk_enable(codec, 1, true);
...
static void msm_mi2s_snd_shutdown(struct snd_pcm_substream *substream)
{
	...
	if (card->aux_dev && substream->stream ==
		...
		if (ret < 0) {
			ad_loge("%s: failed to disable mclk for wsa %d\n",
				__func__, ret);
		}
	}
	if (atomic_read(&pdata->mclk_rsc_ref) > 0) {
...
static int msm_prim_auxpcm_startup(struct snd_pcm_substream *substream)
{
	...
	if (ret < 0)
		ad_loge("%s(): configure gpios failed = %s\n",
				__func__, "quat_i2s");
	return ret;
}
...
static int msm_sec_mi2s_snd_startup(struct snd_pcm_substream *substream)
{
	...
	if ((pdata->ext_pa & SEC_MI2S_ID) == SEC_MI2S_ID) {
		...
		ad_logd("%s(): SEC I2S gpios turned on  = %s\n", __func__,
				"sec_i2s");
...
static int msm_quat_mi2s_snd_startup(struct snd_pcm_substream *substream)
{
	...
	if (atomic_inc_return(&quat_mi2s_clk_ref) == 1) {
		...
		if (ret < 0)
			ad_logd("%s: set fmt codec dai failed\n", __func__);
	}
	return ret;
err:
...
static struct snd_soc_dai_link msm8952_dai_tmp = {
...
static bool msm8952_swap_gnd_mic(struct snd_soc_codec *codec)
{
	...
	ad_logd("%s: configure gpios for US_EU\n", __func__);
		...
		struct msm8916_asoc_mach_data *pdata)
{
	...
	if (IS_ERR(pdata->wsa_switch_supply.supply)) {
		...
		ad_dev_loge(dev, "Failed to get wsa switch supply: err = %d\n",
					ret);
		return ret;
	}
	...
	ret = regulator_set_voltage(pdata->wsa_switch_supply.supply,
		prop_val, prop_val);
	if (ret) {
		ad_dev_loge(dev, "Setting voltage failed for regulator %s err = %d\n",
	...
	ret = regulator_set_optimum_mode(pdata->wsa_switch_supply.supply,
		prop_val);
	if (ret < 0) {
		ad_dev_loge(dev, "Setting current failed for regulator %s err = %d\n",
			switch_supply_str, ret);
		pdata->wsa_switch_supply.supply = NULL;
		return ret;
	}
...
static int msm8952_asoc_machine_probe(struct platform_device *pdev)
{
...
#endif
	if (ext_codec == true) {
		ad_loge("%s: ext codec setprop is true\n", __func__);
		return -EINVAL;
	}
	...
	if (!muxsel) {
		ad_dev_loge(&pdev->dev, "MUX addr invalid for MI2S\n");
		ret = -ENODEV;
		goto err1;
	}
	...
	if (pdata->vaddr_gpio_mux_mic_ctl == NULL) {
		ad_loge("%s ioremap failure for muxsel virt addr\n",
				__func__);
		ret = -ENOMEM;
		goto err1;
	}
	...
	if (num_strings > 0) {
		if (wsa881x_get_probing_count() < 2) {
			...
			if (ret < 0) {
				ad_loge("%s: failed to init wsa_switch vdd supply %d\n",
						__func__, ret);
				goto err;
			}
	...
	for (i = 0; i < num_strings; i++) {
		...
		else if (!strcmp(ext_pa_str, "quaternary"))
		{
    		...
    		ad_dev_logn(&pdev->dev, "using smart pa\n");
    		memcpy(&msm8952_dai[26], &msm8952_dai_tmp, sizeof(struct snd_soc_dai_link));
    		msm8952_dai[39].codec_dai_name = "max98925-aif1";
        	msm8952_dai[39].codec_name = "max98925.8-0031";
        	msm8952_dai[40].codec_dai_name = "max98925-aif1";
        	msm8952_dai[40].codec_name = "max98925.8-0031";
		}
	...
	if (ret < 0) {
		ad_loge("%s: failed to is_us_eu_switch_gpio_support %d\n",
				__func__, ret);
		goto err;
	}
	...
	if (ret < 0)
		ad_logd("%s:  doesn't support external speaker pa\n",
				__func__);
...
static int codec_detector_init_sysfs(void)
{
	...
	attr_group = kzalloc(sizeof(*(attr_group)),
				GFP_KERNEL);
	if (!attr_group) {
		ad_loge("%s: malloc attr_group failed\n",
						__func__);
		ret = -ENOMEM;
		goto error_return;
	}
	...
	if (!boot_cdc_det) {
		ad_loge("%s: sysfs create and add failed\n",
						__func__);
		ret = -ENOMEM;
		goto error_return;
	}
	...
	if (ret) {
		ad_loge("%s: sysfs create group failed %d\n",
							__func__, ret);
		goto error_return;
	}
...
static int __init codec_detector_init(void)
{
	...
	if (ret != 0) {
		ad_loge("%s: Error in initing sysfs\n", __func__);
		return ret;
	}
	return 0;
}
