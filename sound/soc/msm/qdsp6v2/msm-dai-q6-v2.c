		...
		struct msm_dai_q6_mi2s_hdmi_dai_data *ctrl_pdata)
{
	...
	if (!IS_ERR_OR_NULL(ctrl_pdata->dba_data)) {
		...
		ad_loge("%s: error in registering audio client %d\n",
				__func__, ret);
		ret = -EINVAL;
	}
#endif
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!dai_data->hdmi_state) {
		ad_logn("%s: hdmi is not connected yet\n", __func__);
		return -EINVAL;
	}
...
static int msm_dai_q6_dai_mi2s_hdmi_probe(struct snd_soc_dai *dai)
{
	...
	for (i = 0; i < ARRAY_SIZE(hdmi_config_controls); i++) {
		...
		rc = snd_ctl_add(dai->card->snd_card,
				snd_ctl_new1(kcontrol, dai_data));
		if (IS_ERR_VALUE(rc)) {
			ad_dev_loge(dai->dev, "%s: err in adding ctrls = %d\n",
				__func__, rc);
			goto err;
		}
	}
	...
	rc = msm_dai_q6_dai_add_route(dai);
	if (rc < 0) {
		ad_dev_loge(dai->dev, "%s: err in adding routes = %d\n",
			__func__, rc);
		goto err;
	}
	...
	if (rc < 0) {
		ad_dev_loge(dai->dev, "%s: err in hdmi init = %d\n",
			__func__, rc);
		goto err;
	}
	return rc;
					...
					struct snd_soc_dai *dai)
{
	...
	ad_dev_logd(dai->dev, "%s: afe port id = 0x%x\n"
		"dai_data->channels = %u sample_rate = %u\n", __func__,
...
static int __init msm_dai_q6_init(void)
{
	...
	if (rc) {
		ad_loge("%s: fail to register dai HDMI\n", __func__);
