		      ...
		      void *data, uint32_t size)
{
	...
	ad_logd("DTS_EAGLE_ADM: %s - port id %i, copp idx %i, param id 0x%X size %u\n",
	...
	if (p_idx < 0) {
		ad_loge("DTS_EAGLE_ADM: %s: invalid port index 0x%x, port id 0x%x\n",
			__func__, p_idx, port_id);
		return -EINVAL;
	}

	if (copp_idx < 0 || copp_idx >= MAX_COPPS_PER_PORT) {
		ad_loge("DTS_EAGLE_ADM: %s: Invalid copp_idx: %d\n", __func__,
			copp_idx);
		return -EINVAL;
	}
	...
	if ((ret < 0) ||
		...
		ad_loge("DTS_EAGLE_ADM - %s: ion alloc of size %zu too small for size requested %u\n",
		      ...
		      void *srs_params)
{
	...
	case SRS_ID_GEQ: {
		...
		memcpy(geq_params, srs_params,
			sizeof(struct srs_trumedia_params_GEQ));
		ad_logd("SRS - %s: GEQ params prepared\n", __func__);
		break;
	}
	default:
		goto fail_cmd;
...
int adm_set_multi_ch_map(char *channel_map, int path)
{
	...
	if (path == ADM_PATH_PLAYBACK) {
		...
		ad_loge("%s: invalid attempt to set path %d\n", __func__, path);
		return -EINVAL;
	}
...
int adm_get_multi_ch_map(char *channel_map, int path)
{
	...
	if (path == ADM_PATH_PLAYBACK) {
		...
		ad_loge("%s: invalid attempt to get path %d\n", __func__, path);
		return -EINVAL;
	}
	...
	if (data->payload_size) {
			...
			case ADM_CMD_GET_PP_PARAMS_V5:
				...
				if (client_id ==
					...
					this_adm.sourceTrackingData.
						apr_cmd_status = payload[1];
					if (payload[1] != 0)
						ad_loge("%s: ADM get param error = %d\n",
							__func__, payload[1]);
		...
		case ADM_CMDRSP_GET_PP_PARAMS_V5:
			...
			if ((payload[0] == 0) && (data->payload_size >
				...
				ad_loge("%s: Out of band case, setting size to %d\n",
					__func__, adm_get_parameters[idx]);
			} else {
				adm_get_parameters[idx] = -1;
...
static void send_adm_cal(int port_id, int copp_idx, int path, int perf_mode,
			 int app_type, int acdb_id, int sample_rate)
{
	ad_logd("%s: port id 0x%x copp_idx %d\n", __func__, port_id, copp_idx);
	     ...
	     int perf_mode, uint16_t bit_width, int app_type, int acdb_id)
{
	...
	if ((topology == ADM_CMD_COPP_OPEN_TOPOLOGY_ID_DTS_HPX) &&
		...
		bit_width = 24;
		ad_logd("%s: Force open adm in 24-bit for DTS HPX topology 0x%x\n",
			__func__, topology);
	}
	...
	if ((topology == SRS_TRUMEDIA_TOPOLOGY_ID) &&
		...
		if ((topology == ADM_CMD_COPP_OPEN_TOPOLOGY_ID_DTS_HPX) &&
			...
			res = adm_memory_map_regions(
				      ...
				      1);
			if (res < 0)
				ad_loge("%s: DTS_EAGLE mmap did not work!",
					__func__);
		}
...
int adm_matrix_map(int path, struct route_payload payload_map, int perf_mode)
{
	...
	if (!ret) {
		...
		panic("@@routing for payload_map failed@@");
#endif
		goto fail_cmd;
		...
		for (i = 0; i < payload_map.num_copps; i++) {
			...
			if (!test_bit(ADM_STATUS_CALIBRATION_REQUIRED,
				(void *)&this_adm.copp.adm_status[port_idx]
								[copp_idx])) {
				ad_logd("%s: adm copp[0x%x][%d] already sent",
						__func__, port_idx, copp_idx);
				continue;
			}
			...
			struct sound_focus_param soundFocusData)
{
	...
	ad_logd("%s: Enter, port_id %d, copp_idx %d\n",
		  __func__, port_id, copp_idx);
	...
	ret = wait_event_timeout(this_adm.copp.wait[port_idx][copp_idx],
		...
		msecs_to_jiffies(TIMEOUT_MS));
	if (!ret) {
		ad_loge("%s: Set params timed out\n", __func__);
	...
	if (this_adm.sourceTrackingData.apr_cmd_status != 0) {
		ad_loge("%s - set params returned error [%s]\n",
			__func__, adsp_err_get_err_str(
			this_adm.sourceTrackingData.apr_cmd_status));
			...
			struct sound_focus_param *soundFocusData)
{
	...
	if (!params_value) {
		ad_loge("%s, params memory alloc failed\n", __func__);
	...
	ret = adm_get_params_v2(port_id, copp_idx,
				...
				ADM_CLIENT_ID_SOURCE_TRACKING);
	if (ret != 0) {
		ad_loge("%s: get parameters failed ret:%d\n", __func__, ret);
	...
	if (this_adm.sourceTrackingData.apr_cmd_status != 0) {
		ad_loge("%s - get params returned error [%s]\n",
			__func__, adsp_err_get_err_str(
			this_adm.sourceTrackingData.apr_cmd_status));
...
static int adm_source_tracking_alloc_map_memory(void)
{
	...
	ret = msm_audio_ion_alloc("SOURCE_TRACKING",
				  ...
				  &this_adm.sourceTrackingData.memmap.kvaddr);
	if (ret != 0) {
		ad_loge("%s: failed to allocate memory\n", __func__);
	...
	ret = adm_memory_map_regions(&this_adm.sourceTrackingData.memmap.paddr,
			...
			1);
	if (ret < 0) {
		ad_loge("%s: failed to map memory, paddr = 0x%p, size = %d\n",
			__func__,
	...
	ret = 0;
	ad_logd("%s: paddr = 0x%p, size = %d, mem_map_handle = 0x%x\n",
			    ...
			    struct source_tracking_param *sourceTrackingData)
{
	...
	p_idx = adm_validate_and_get_port_index(port_id);
	if (p_idx < 0) {
		ad_loge("%s - invalid port index %i, port id %i, copp idx %i\n",
			__func__, p_idx, port_id, copp_idx);
	...
	ret = apr_send_pkt(this_adm.apr, (uint32_t *)&admp);
	if (ret < 0) {
		ad_loge("%s - failed to get Source Tracking Params\n",
			__func__);
	...
	ret = wait_event_timeout(this_adm.copp.wait[p_idx][copp_idx],
			...
			msecs_to_jiffies(TIMEOUT_MS));
	if (!ret) {
		ad_loge("%s - get params timed out\n", __func__);
