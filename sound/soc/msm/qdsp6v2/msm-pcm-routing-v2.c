				   ...
				   int channels)
{
	...
	case ADM_CMD_COPP_OPEN_TOPOLOGY_ID_AUDIOSPHERE:
		ad_logd("%s: TOPOLOGY_ID_AUDIOSPHERE\n", __func__);
	...
	if (set) {
		...
		if (msm_bedais[reg].active && fdai->strm_id !=
			...
			adm_close(msm_bedais[reg].port_id, fdai->perf_mode,
				  idx);
			ad_logd("%s: copp: %ld, reset idx bit fe:%d, type: %d, be:%d topology=0x%x\n",
				     ...
				     struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ret != 0) {
		ad_loge("%s: Error setting Sound Focus Params, err=%d\n",
			  __func__, ret);
				     ...
				     struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ret != 0) {
		ad_loge("%s: Error getting Sound Focus Params, err=%d\n",
			  __func__, ret);
					 ...
					 struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ret != 0) {
		ad_loge("%s: Error getting Source Tracking Params, err=%d\n",
			  __func__, ret);
					 ...
					 int *copp_idx)
{
	...
	if (i >= MSM_FRONTEND_DAI_MM_SIZE) {
		ad_loge("%s: Invalid FE, exiting\n", __func__);

		ret = -EINVAL;
		goto done;
	}
	*copp_idx = idx;
	ad_logd("%s: copp_idx=%d\n", __func__, *copp_idx);
					    ...
					    const char *prefix, int *port_id)
{
	...
	if (!strcmp(kcontrol->id.name + strlen(prefix),
		...
		*port_id = AFE_PORT_ID_TERTIARY_MI2S_TX;
		...
		ad_loge("%s: mixer ctl name=%s, could not derive valid port id\n",
			__func__, kcontrol->id.name);

		ret = -EINVAL;
		goto done;
	}
	ad_logd("%s: mixer ctl name=%s, derived port_id=%d\n",
				     ...
				     struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ret != 0) {
		ad_loge("%s: Error in deriving port id, err=%d\n",
			  __func__, ret);
	...
	ret = msm_audio_get_copp_idx_from_port_id(port_id, SESSION_TYPE_TX,
					    &copp_idx);
	if (ret != 0) {
		ad_loge("%s: Could not get copp idx for port_id=%d\n",
			__func__, port_id);
...
static int msm_pcm_routing_close(struct snd_pcm_substream *substream)
{
	...
	for_each_set_bit(i, &bedai->fe_sessions, MSM_FRONTEND_DAI_MM_SIZE) {
		...
		if (fdai->strm_id != INVALID_SESSION) {
			...
			ad_logd("%s: copp:%ld,idx bit fe:%d, type:%d,be:%d topology=0x%x\n",
