...
#define PIN_STATUS_FLOAT  2
#define PIN_STATUS_ERR    (-1)
#define SPK_BOX_IS_AAC    0
#define SPK_BOX_IS_GOER   1
#define SPK_BOX_IS_GD     2
#define SPK_BOX_IS_NONE   (-1)
...
int get_pa_box_id(struct platform_device *pdev, struct pinctrl *pinctrl)
{
	...
	int gpio_box_id_1;
	int ret;
	int box_value_pull_up_0 = -1;
	int box_value_pull_up_1 = -1;
	int box_value_pull_down_0 = -1;
	int box_value_pull_down_1 = -1;
	int pin_status0 = PIN_STATUS_ERR;
	int pin_status1 = PIN_STATUS_ERR;
	//pull_up   pull_down
	int pin_status[2][2] = 
	{
	    {PIN_STATUS_LOW, PIN_STATUS_ERR}, 
	    {PIN_STATUS_FLOAT, PIN_STATUS_HIGH}
	};
	
	//gpio_0   gpio_1
	int box_status[3][3] = 
	{
	    {SPK_BOX_IS_GD,   SPK_BOX_IS_NONE,  SPK_BOX_IS_GOER},
	    {SPK_BOX_IS_NONE, SPK_BOX_IS_NONE,  SPK_BOX_IS_NONE},
	    {SPK_BOX_IS_AAC, SPK_BOX_IS_NONE,  SPK_BOX_IS_NONE}
	};

    gpio_box_id_0 = of_get_named_gpio(pdev->dev.of_node, "qcom,box-id-0", 0);
    if (gpio_box_id_0 < 0) 
    {
        ad_loge("%s: failed to get gpio_box_id_0\n", __func__);
        return SPK_BOX_IS_NONE;
    }
    
    gpio_box_id_1 = of_get_named_gpio(pdev->dev.of_node, "qcom,box-id-1", 0);
    if (gpio_box_id_1 < 0) 
    {
        ad_loge("%s: failed to get gpio_box_id_1\n", __func__);
        return SPK_BOX_IS_NONE;
    }
    
    ret = gpio_request(gpio_box_id_0, "gpio_box_id_0");
    if (ret) 
    {
        ad_loge("%s: unable to request gpio_box_id_0\n", __func__);
        gpio_free(gpio_box_id_0);
        return SPK_BOX_IS_NONE;
    }

    ret = gpio_request(gpio_box_id_1, "gpio_box_id_1");
    if (ret) 
    {
        ad_loge("%s: unable to request gpio_box_id_1\n", __func__);
        gpio_free(gpio_box_id_1);
        return SPK_BOX_IS_NONE;
    }
    ...
    udelay(10);
    gpio_direction_input(gpio_box_id_0);
    box_value_pull_up_0 = gpio_get_value(gpio_box_id_0);
    ad_logn("%s: box_value_0 up = %d\n", __func__, box_value_pull_up_0);
    box_value_pull_up_0 = (box_value_pull_up_0 == PIN_VOLTAGE_LOW) ? PIN_VOLTAGE_LOW : PIN_VOLTAGE_HIGH;
    gpio_direction_input(gpio_box_id_1);
    box_value_pull_up_1 = gpio_get_value(gpio_box_id_1);
    ad_logn("%s: box_value_1 up = %d\n", __func__, box_value_pull_up_1);
    box_value_pull_up_1 = (box_value_pull_up_1 == PIN_VOLTAGE_LOW) ? PIN_VOLTAGE_LOW : PIN_VOLTAGE_HIGH;

    pinctrl_select_state(pinctrl, pin_sleep);
    udelay(10);
    gpio_direction_input(gpio_box_id_0);
    box_value_pull_down_0 = gpio_get_value(gpio_box_id_0);
    ad_logn("%s: box_value_0 down = %d\n", __func__, box_value_pull_down_0);
    box_value_pull_down_0 = (box_value_pull_down_0 == PIN_VOLTAGE_LOW) ? PIN_VOLTAGE_LOW : PIN_VOLTAGE_HIGH;
    gpio_direction_input(gpio_box_id_1);
    box_value_pull_down_1 = gpio_get_value(gpio_box_id_1);
    ad_logn("%s: box_value_1 down = %d\n", __func__, box_value_pull_down_1);
    box_value_pull_down_1 = (box_value_pull_down_1 == PIN_VOLTAGE_LOW) ? PIN_VOLTAGE_LOW : PIN_VOLTAGE_HIGH;
    gpio_free(gpio_box_id_0);
    gpio_free(gpio_box_id_1);

    pin_status0 = pin_status[box_value_pull_up_0][box_value_pull_down_0];
    pin_status1 = pin_status[box_value_pull_up_1][box_value_pull_down_1];
    ad_logn("%s: pin_status0 = %d, pin_status1 = %d\n", __func__, pin_status0, pin_status1);
    if((pin_status0 == PIN_STATUS_ERR) || (pin_status1 == PIN_STATUS_ERR))
    {
        return SPK_BOX_IS_NONE;
    }
    
    return box_status[pin_status0][pin_status1];
}
