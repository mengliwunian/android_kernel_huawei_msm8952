...
void  huawei_save_nff_kernel_init(void)
{
    ...
    checksum = (uint32_t*)(boot_log_virt + MAGIC_NUMBER_SIZE);
	...
	pr_notice("NFF: magic number matching  %x %x. Disabled NFF\n", 
				  *magic, HUAWEI_BOOT_LOG_ADDR);
       pr_notice("NFF: boot_log_virt addr = %p \n", boot_log_virt);
	boot_log = (struct boot_log_struct *)(boot_log_virt + MAGIC_NUMBER_SIZE + CHECKSUM_SIZE);
	pr_notice("NFF: boot_log=%p\n", (void*)boot_log);
