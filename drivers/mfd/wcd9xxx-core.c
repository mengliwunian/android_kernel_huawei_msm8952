	...
	if (wcd9xxx->prev_pg_valid) {
		...
		if (prev_pg_num != pg_num) {
			ret = wcd9xxx->write_dev(
					...
					(void *) &pg_num, false);
			if (ret < 0)
				ad_loge("page write error, pg_num: 0x%x\n",
					pg_num);
			else {
				wcd9xxx->prev_pg = pg_num;
				ad_dev_logd(wcd9xxx->dev, "%s: Page 0x%x Write to 0x00\n",
					__func__, pg_num);
			}
		}
	} else {
			    ...
			    void *val, size_t val_size)
{
	...
	if (!wcd9xxx) {
		ad_dev_loge(dev, "%s: wcd9xxx is NULL\n", __func__);
		return -EINVAL;
	}
	if (!reg || !val) {
		ad_dev_loge(dev, "%s: reg or val is NULL\n", __func__);
		return -EINVAL;
	}

	if (reg_size != REG_BYTES) {
		ad_dev_loge(dev, "%s: register size %zd bytes, not supported\n",
			__func__, reg_size);
		return -EINVAL;
	}
	...
	if (ret < 0)
		ad_dev_loge(dev, "%s: Codec read failed (%d), reg: 0x%x\n",
			__func__, ret, rreg);
	else {
		for (i = 0; i < val_size; i++)
			ad_dev_logd(dev, "%s: Read 0x%02x from 0x%x\n",
				...
				const void *val, size_t val_size)
{
	...
	for (i = 0; i < val_size; i++)
		ad_dev_logd(dev, "Write %02x to 0x%x\n", ((u8 *)val)[i],
			rreg + i);

	ret = wcd9xxx->write_dev(wcd9xxx, c_reg, val_size, (void *) val,
				 false);
	if (ret < 0)
		ad_dev_loge(dev, "%s: Codec write failed (%d)\n", __func__, ret);
				     ...
				     unsigned short reg, u8 mask, u8 val)
{
	...
	if (wcd9xxx->using_regmap)
		...
		if (ret < 0) {
			ad_dev_loge(wcd9xxx->dev, "%s: Codec read 0x%x failed\n",
				__func__, reg);
			goto err;
		}
		tmp = orig & ~mask;
				    ...
				    enum wcd_power_regions region)
{
	...
	if ((region < 0) || (region >= WCD9XXX_MAX_PWR_REGIONS)) {
		ad_dev_loge(wcd9xxx->dev, "%s: region index %d out of bounds\n",
			__func__, region);
		return -EINVAL;
	}
	if (!wcd9xxx->wcd9xxx_pwr[region]) {
		ad_dev_loge(wcd9xxx->dev, "%s: memory not created for region: %d\n",
			__func__, region);
		return -EINVAL;
	}
			    ...
			    unsigned int size, bool interface)
{
	...
	ret = wcd9xxx_page_write(wcd9xxx, &reg);
	if (ret) {
		ad_loge("%s: Page write error for reg: 0x%x\n",
			__func__, reg);
		goto err;
	}
	...
	ret = slim_bulk_msg_write(interface ?
				  ...
				  NULL, NULL);
	if (ret)
		ad_loge("%s: Error, Codec bulk write failed (%d)\n",
			__func__, ret);
...
static int wcd9xxx_regmap_init_cache(struct wcd9xxx *wcd9xxx)
{
	...
	if (!regmap_config) {
		ad_dev_loge(wcd9xxx->dev, "regmap config is not defined\n");
		return -EINVAL;
	}

	rc = regmap_reinit_cache(wcd9xxx->regmap, regmap_config);
	if (rc != 0) {
		ad_dev_loge(wcd9xxx->dev, "%s:Failed to reinit register cache: %d\n",
			__func__, rc);
	}
					  ...
					  loff_t *ppos)
{
	...
	for (i = (int) *ppos / 12; i <= SLIM_MAX_REG_ADDR; i++) {
		...
		if (copy_to_user((ubuf + total), tmp_buf, len)) {
			ad_loge("%s: fail to copy reg dump\n", __func__);
			total = -EFAULT;
			goto copy_err;
		}
		*ppos += len;
				...
				size_t count, loff_t *ppos)
{
	...
	if (!strcmp(access_str, "slimslave_peek")) {
		...
		ad_loge("%s: %s not permitted to read\n", __func__, access_str);
		ret_cnt = -EPERM;
	}
...
static int codec_debug_process_cdc_power(char *lbuf)
{
	...
	if (wcd9xxx_get_intf_type() != WCD9XXX_INTERFACE_TYPE_SLIMBUS) {
		ad_loge("%s: CODEC is not in SLIMBUS mode\n", __func__);
		rc = -EPERM;
		goto error_intf;
	}
	...
	struct wcd9xxx_micbias_setting *micbias)
{
	...
	ad_dev_logd(dev, "micb1_mv %u micb2_mv %u micb3_mv %u micb4_mv %u",
...
static int wcd9xxx_slim_probe(struct slim_device *slim)
{
	...
	if (slim->dev.of_node) {
		...
		if (of_id) {
			...
			ad_dev_logn(&slim->dev, "%s: codec type is %d\n",
				 __func__, wcd9xxx->type);
		}
	} else {
		ad_dev_logn(&slim->dev, "%s: dev.of_node is NULL, default to WCD9XXX\n",
			 __func__);
		wcd9xxx->type = WCD9XXX;
	}
	...
	if (wcd9xxx->using_regmap) {
		...
		if (IS_ERR(wcd9xxx->regmap)) {
			...
			ad_dev_loge(&slim->dev, "%s: Failed to allocate register map: %d\n",
				__func__, ret);
			goto err_codec;
		}
	}
						...
						void *data)
{
	...
	for (i = 0; i < wcd9xxx->num_of_supplies; i++) {
		...
		regulator_set_voltage(wcd9xxx->supplies[i].consumer, 0,
			pdata->regulator[i].max_uV);
		regulator_set_optimum_mode(wcd9xxx->supplies[i].consumer, 0);
		ad_dev_logd(wcd9xxx->dev, "Regulator %s set optimum mode\n",
				 wcd9xxx->supplies[i].supply);
	}
	return ret;
}
