...
extern void alarm_sysfs_add_device(struct rtc_device *rtc);
extern void alarm_sysfs_del_device(struct rtc_device *rtc);
#endif
...
#ifdef CONFIG_HUAWEI_KERNEL
    alarm_sysfs_add_device(rtc_dd->rtc);
#endif
...
#ifdef CONFIG_HUAWEI_KERNEL
    alarm_sysfs_del_device(rtc_dd->rtc);
#endif
	rtc_device_unregister(rtc_dd->rtc);
	dev_set_drvdata(&spmi->dev, NULL);
