...
static const char *hw_cpu_distr_list = NULL;
static const char hw_cpu_def_list[] = "B0123,L4567";
static int hw_cpu_distr_parse_dt(struct device *dev,
						char *tbl_name)
{
	int ret = 0;
	/* Parse list of usable CPU frequencies. */
	...
	ret = of_property_read_string(dev->of_node, tbl_name, &hw_cpu_distr_list);
...
DT_PARSE_ERR:
	/* Use default setings */
	hw_cpu_distr_list = hw_cpu_def_list;

	return 0;
}

static ssize_t hw_cpu_distrib_show(struct device *dev,
				  struct device_attribute *attr,
				  char *buf)
{
	return snprintf(buf, strlen(hw_cpu_distr_list)+1,"%s\n", hw_cpu_distr_list);
}
static DEVICE_ATTR(hw_cpu_distrib, 0400, hw_cpu_distrib_show, NULL);
...
static int __init msm_cpufreq_probe(struct platform_device *pdev)
{
	...
	rc = device_create_file(cpu_subsys.dev_root, &dev_attr_hw_cpu_distrib);
	if (rc) {
		pr_err ("Cannot create the hw_cpu_distrib\n");
		return rc;
	}
	...
	hw_cpu_distr_parse_dt(dev, "hw_cpu_distri");
	return 0;
}
