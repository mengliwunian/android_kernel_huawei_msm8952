	...
	if(1 == rwlog_enable_flag)
	{
		if (mrq->data) {
			if (mrq->data->flags & MMC_DATA_READ) {
				...
				pr_err("mlhxxx-%s:     blk_addr %d blksz %d blocks %d flags %08x tsac %lu ms nsac %d (r blocks %ld)\n",
			...
			if (mrq->data->flags & MMC_DATA_WRITE) {
				...
				pr_err("mlhxxx-%s:     blk_addr %d blksz %d blocks %d flags %08x tsac %lu ms nsac %d (w blocks %ld)\n",
				mmc_hostname(host), mrq->cmdq_req->blk_addr, mrq->data->blksz,
				mrq->data->blocks, mrq->data->flags,
				mrq->data->timeout_ns / NSEC_PER_MSEC,
...
u32 mmc_select_voltage(struct mmc_host *host, u32 ocr)
{
	...
	if (bit) {
		...
		EMMCSD_LOG_ERR("%s: host doesn't support card's voltages\n",
				mmc_hostname(host));
		ocr = 0;
	}
...
int mmc_resume_bus(struct mmc_host *host)
{
	...
	spin_unlock_irqrestore(&host->lock, flags);
	EMMCSD_LOG_INFO("%s: Starting deferred resume\n", mmc_hostname(host));
	mmc_bus_get(host);
	if (host->bus_ops && !host->bus_dead) {
		...
		if (err) {
			EMMCSD_LOG_ERR("%s: error %d during resume "
					    "(card was removed?)\n",
					    mmc_hostname(host), err);
			err = 0;
		}
	...
	EMMCSD_LOG_INFO("%s: Deferred resume completed\n", mmc_hostname(host));
	return 0;
#else
	unsigned long flags;
