						...
						struct sdhci_host *host)
{
	...
	if (of_get_property(np, "huawei,support-polling", NULL))
	{
		...
		EMMCSD_LOG_INFO("polling has enable sucess.\n");
...
static int sdhci_msm_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		{
			EMMCSD_LOG_INFO("the voltage of gpio is high when insert the sdcard.\n");
		}
		else if (sdhci_msm_set_gpio_info(msm_host->pdata) == 0)
		{
			EMMCSD_LOG_INFO("the voltage of gpio is low when insert the sdcard.\n");
		}
		else
		{
			EMMCSD_LOG_ERR("sdhci_msm_set_gpio_info failed.\n");
		}
#endif
	} else {
...
static int sdhci_msm_resume(struct device *dev)
{
	...
	if (gpio_is_valid(msm_host->pdata->status_gpio)) {
		...
		ret = mmc_gpio_request_cd(msm_host->mmc,
				msm_host->pdata->status_gpio);
		if (ret)
			EMMCSD_LOG_ERR("%s: %s: Failed to request card detection IRQ %d\n",
					mmc_hostname(host->mmc), __func__, ret);
	}
#endif
