...
#define QPNP_VIB_GPIO_MODE_CTL(base)		(base + 0x40)
#define QPNP_VIB_GPIO_HIGH		 BIT(0)
#define QPNP_VIB_GPIO_OUTPUT  (0x1<<4)
...
static int qpnp_vibrator_config(struct qpnp_vib *vib)
{
	...
	int rc;
	if(vib->use_gpio){
		/*Configure pmu of gpio8 mode register*/
		rc = qpnp_vib_read_u8(vib, &reg, QPNP_VIB_GPIO_MODE_CTL(vib->base));
		if (rc < 0)
			return rc;
		/*Configure gpio as an output*/
		reg |= QPNP_VIB_GPIO_OUTPUT;
		reg &= ~QPNP_VIB_GPIO_HIGH;
		rc = qpnp_vib_write_u8(vib, &reg, QPNP_VIB_GPIO_MODE_CTL(vib->base));
		if (rc < 0)
			return rc;
	}
	else{
...
static int qpnp_vib_set(struct qpnp_vib *vib, int on)
{
	...
	if (on) {
		if(vib->use_gpio){
			val = vib->reg_en_ctl;
			val |= QPNP_VIB_GPIO_HIGH;
			rc = qpnp_vib_write_u8(vib, &val,
					QPNP_VIB_GPIO_MODE_CTL(vib->base));
			if (rc < 0)
				return rc;
			vib->reg_en_ctl = val;
		}
...
static void qpnp_vib_enable(struct timed_output_dev *dev, int value)
{
	...
	dev_err(&vib->spmi->dev, "%s:open the vibrator,time = %dms\n",__func__,value);
	if (value == 0)
		vib->state = 0;
	else {
...
static int qpnp_vib_parse_dt(struct qpnp_vib *vib)
{
	...
	vib->active_low = of_property_read_bool(spmi->dev.of_node,
				"qcom,active-low");
	vib->use_gpio = false;
	vib->use_gpio = of_property_read_bool(spmi->dev.of_node,
				"vibrator,use-gpio");
	return 0;
}
