				...
				uint32_t src_port, void *priv)
{
	...
	if (dest_id == APR_DEST_QDSP6) {
		...
		if (apr_get_modem_state() == APR_SUBSYS_DOWN) {
			if (is_modem_up) {
				ad_loge("%s: modem shutdown due to SSR, ret",
					__func__);
				return NULL;
			}
		...
		ad_loge("%s: Invalid destination id %d\n", __func__,
				dest_id);
		return NULL;
	}
...
int apr_set_subsys_state(void)
{
	...
	if (apr_dsp_func.apr_set_subsys_state) {
		...
		ad_loge("%s: invalid function ptr\n", __func__);
		ret = -EINVAL;
	}
	return ret;
}
...
static int apr_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_string(pdev->dev.of_node,
		...
		&dsp_type);
	if (ret || !dsp_type) {
		ad_dev_loge(&pdev->dev, "%s: Looking up %s property failed\n",
		__func__, "qcom,apr-dest-type");
		return -EINVAL;
	}
	if (!strcmp("ADSP", dsp_type)) {
		ad_dev_logn(&pdev->dev, "%s: destination is ADSP\n", __func__);
		ret = apr_get_v2_ops(&apr_dsp_func);
		if (ret) {
			ad_dev_loge(&pdev->dev, "%s error get adsp ops %d\n",
					__func__, ret);
			return ret;
		}
	} else if (!strcmp("MDSP", dsp_type)) {
		ad_dev_logn(&pdev->dev, "%s: destination is modem\n", __func__);
		ret = apr_get_v3_ops(&apr_dsp_func);
		if (ret) {
			ad_dev_loge(&pdev->dev, "%s: error get mdsp ops %d\n",
					__func__, ret);
			return ret;
		}
	} else if (!strcmp("Dynamic", dsp_type)) {
		ad_dev_logn(&pdev->dev, "%s: using service registry\n", __func__);
	} else {
		ad_dev_loge(&pdev->dev, "%s: Invalid destination type\n",
				__func__);
		return -EINVAL;
	}
	...
	if (!apr_reset_workqueue) {
		ad_loge("%s: work queue creation failed\n", __func__);
		apr_cleanup();
		return -ENOMEM;
	}
	ret = apr_set_subsys_state();
	if (ret)
		ad_dev_loge(&pdev->dev, "%s: apr_set_subsys_state failed ret = %d\n",
				__func__, ret);
	...
	if (ret) {
		ad_dev_loge(&pdev->dev, "%s: subsys_notif_register failed ret = %d\n",
				__func__, ret);
	}
