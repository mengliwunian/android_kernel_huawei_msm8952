...
void msm_isp_process_error_info(struct vfe_device *vfe_dev)
{
	...
	if (error_info->error_count == 1 ||
		...
		ISP_HW_DBG("%s:%d] vfe_dev %p id %d ,error_info:%d,%d,%d,%d\n", __func__,__LINE__, 
			vfe_dev, vfe_dev->pdev->id,error_info->error_mask0,error_info->error_mask1,
			error_info->camif_status,error_info->violation_status);
		vfe_dev->hw_info->vfe_ops.core_ops.
