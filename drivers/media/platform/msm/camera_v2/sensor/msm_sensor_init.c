...
{
	int rc;
	/*sometimes, mediaserver waits the event timeout when daemon starting too later
	   caused by the system boot on in a low speed, change the tm from 10s to 15s*/
	int tm = 15000;
...
int32_t msm_get_probe_hw_product_name(void *setting)
{
	...
	if (copy_to_user((void *)setting, (void *)dt_product_name, length)){
...
static int __init msm_sensor_init_module(void)
{
	...
	if (of_property_read_string(of_node, "qcom,product-name",
		...
		pr_err("%s: can't find support product name.\n",__func__);
	}
	else{
