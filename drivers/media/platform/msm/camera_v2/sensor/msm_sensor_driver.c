	...
	struct msm_sensor_info_t *probed_info, char *entity_name)
{
	...
	uint32_t                             is_yuv;
	int                                  i = 0;
	/*use an local ptr to malloc mem of dump_reg_info instead of camera_info->dump_reg_info,
		to avoid free an unzero camera_info->dump_reg_info ptr when goto CSID_TG*/
	...
	if (is_compat_task()) {
		...
		slave_info->otp_vendor_info = compat_ptr(slave_info32->otp_vendor_info);
	...
	if(slave_info->otp_vendor_info){
		otp_vendor_info = kzalloc(sizeof(*otp_vendor_info), GFP_KERNEL);
		if(!otp_vendor_info){
			pr_err("failed, no memory vendor info %p\n",otp_vendor_info);
			goto free_vendor_info;
		}

		camera_info->otp_vendor_info= otp_vendor_info;

		if(copy_from_user(otp_vendor_info, slave_info->otp_vendor_info,
				sizeof(*otp_vendor_info))){
	...
	if (0 !=  slave_info->dump_reg_num && slave_info->dump_reg_info) {
		...
		dump_reg_info = kzalloc( (sizeof (*dump_reg_info)) *
			slave_info->dump_reg_num, GFP_KERNEL);

		if (!dump_reg_info) {
			pr_err("failed: no memory dump reg info %p\n", camera_info->dump_reg_info);
			rc = -ENOMEM;
			goto free_camera_info;
		}
		...
		if (copy_from_user(dump_reg_info,slave_info->dump_reg_info,
					sizeof(*dump_reg_info) * slave_info->dump_reg_num)) {
			...
			for(i = 0; i < (camera_info->dump_reg_num); i++)
				...
				dump_reg_info[i].addr);
		}
	} else {
		pr_err("sensor %s have no dump_reg_info\n",slave_info->sensor_name);
	}
	/*change goto free_camera_info to goto free_dump_info to avoid mem leak*/
