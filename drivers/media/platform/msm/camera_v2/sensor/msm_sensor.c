...
static int msm_sensor_check_cam_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if(!slave_info->cam_id_info){
		pr_err("%s, %s, no need to check camera id\n", __func__, sensor_name);
		return 0;
	}
	sensor_ctrl->pinctrl = devm_pinctrl_get(s_ctrl->sensordata->power_info.dev);
	if (IS_ERR_OR_NULL(sensor_ctrl->pinctrl)){
		pr_err( "Unable to get pinctrl handle\n");
		return -ENODEV;
	}
	cam_gpio =s_ctrl->sensordata->power_info.gpio_conf->gpio_num_info->gpio_num[SENSOR_GPIO_CAM_ID];
	...
	if (IS_ERR_OR_NULL(pin_up) && IS_ERR_OR_NULL(pin_down)){
		gpio_direction_input(cam_gpio);
		mcam_id=gpio_get_value(cam_gpio);
		if(mcam_id == slave_info->cam_id_info->cam_expected_id){
			rc=0;
			pr_err("%s:%s gpio %d except value:%d match",__func__,sensor_name,cam_gpio,mcam_id);
			return rc;
		}else{
			pr_err("%s:%s gpio %d value:%d not match",__func__,sensor_name,cam_gpio,mcam_id);
		...
		udelay(10);
		gpio_direction_input(cam_gpio);
		pullup_read = gpio_get_value(cam_gpio);

		pinctrl_select_state(sensor_ctrl->pinctrl,pin_down);
		udelay(10);
		gpio_direction_input(cam_gpio);
		pulldown_read = gpio_get_value(cam_gpio);
...
static int msm_sensor_check_otp_vendor_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if( (vendor_info&0xF0)>>4 == otp_vendor_info->expect_vendor_id ){
		pr_err("%s, probe vendor_id success,vendor_info:0x%x, expect_vendor_id:0x%x.\n",__func__,
			(vendor_info&0xF0)>>4,otp_vendor_info->expect_vendor_id);
		rc = 0;
	}else{
