...
#define IMX214_MODULE_HUAWEI_ID          0x92//23060146
#define IMX214_SUNNY_MODULE_VCM_OFFSET_START    170
#define IMX214_SUNNY_MODULE_VCM_OFFSET_END       40
#define IMX214_FOXCONN_MODULE_VCM_OFFSET_START  170
#define IMX214_FOXCONN_MODULE_VCM_OFFSET_END     40
#define IMX214_OFILM_MODULE_VCM_OFFSET_START    170
#define IMX214_OFILM_MODULE_VCM_OFFSET_END       40
...
typedef struct
{
    uint16_t af_offset_start;
    uint16_t af_offset_end;
} af_offset;

static af_offset af_offset_defined[] =
{
    {0,0},   //dummy
    {IMX214_SUNNY_MODULE_VCM_OFFSET_START,IMX214_SUNNY_MODULE_VCM_OFFSET_END}, //sunny module vcm offset
    {IMX214_FOXCONN_MODULE_VCM_OFFSET_START,IMX214_FOXCONN_MODULE_VCM_OFFSET_END}, //foxconn module vcm offset
    {0,0},
    {0,0},
    {0,0},
    {IMX214_OFILM_MODULE_VCM_OFFSET_START,IMX214_OFILM_MODULE_VCM_OFFSET_END},//ofilm module vcm offset
};
...
static int32_t imx214_otp_i2c_read_seq(struct msm_sensor_ctrl_t* s_ctrl, uint32_t addr, uint8_t* data, uint32_t num_bytes)
{
    int32_t rc = -EFAULT;
...
static bool imx214_read_otp(struct msm_sensor_ctrl_t* s_ctrl, uint16_t i2c_addr, uint16_t reg, uint8_t* buf, uint16_t count)
{
    ...
    rc = imx214_otp_i2c_read_seq(s_ctrl, reg, buf, count);
    if(rc < 0){
        CMR_LOGE("%s failed ,reg:%d num:%d\n", __func__, reg, count);
        goto OUT;
    }

    for (i = 0; i < count; i++) {
        OTPSUMVAL += buf[i];
        CMR_LOGD("%s: i2c_addr=0x%x start_reg_addr=0x%x, reg_val:0x%x, otpsum:0x%x\n",
            __func__, i2c_addr, reg, buf[i], OTPSUMVAL);
    }
...
static void imx214_otp_set_af(struct msm_sensor_ctrl_t* s_ctrl)
{
    ...
    uint16_t af_offset_start = af_offset_defined[imx214_otp.module_id].af_offset_start;
    uint16_t af_offset_end = af_offset_defined[imx214_otp.module_id].af_offset_end;

    if (af_start <= af_offset_start){
        af_start = 0;
    } else {
        af_start -= af_offset_start;
    }

    af_end += af_offset_end;
...
int imx214_otp_func(struct msm_sensor_ctrl_t* s_ctrl, int index)
{
    ...
    CMR_LOGW("%s, the OTP read and set end.\n", __func__);
