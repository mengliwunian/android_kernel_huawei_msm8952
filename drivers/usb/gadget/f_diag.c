		...
		struct usb_request *req)
{
	...
	{
        print_hex_dump(KERN_INFO, "[USB_LOGS]Data to usb: ", DUMP_PREFIX_ADDRESS,
                          16, 1, req_buffer_part_out, 16, true);
    }
		...
		struct usb_request *req)
{
	...
	if(( 75 == *((u8 *)req->buf)) && (( 201 == *((u8 *)req->buf+1) )||(200 == *((u8 *)req->buf+1))))
	{
        print_hex_dump(KERN_INFO, "[USB_LOGS]Data from usb: ", DUMP_PREFIX_ADDRESS,
                          16, 1, req_buffer_part_in, 16, true);
    }
	/*printf  7B 00 01 02 03 04 05 06 07 08 09 9D 5F 7E */
	if(( 123 == *((u8 *)req->buf)) && (0 == *((u8 *)req->buf+1)))
	{
		...
		void (*notify)(void *, unsigned, struct diag_request *))
{
	...
	pr_info("[USB_LOGS] %s,usb diag is open.\n",__func__);
...
void usb_diag_close(struct usb_diag_ch *ch)
{
	...
	unsigned long flags;
	pr_info("[USB_LOGS] %s:usb diag is close.\n",__func__);
	spin_lock_irqsave(&ch_lock, flags);
	ch->priv = NULL;
	ch->notify = NULL;
