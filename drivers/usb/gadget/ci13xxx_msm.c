...
static void ci13xxx_msm_notify_event(struct ci13xxx *udc, unsigned event)
{
	...
	case CI13XXX_CONTROLLER_RESET_EVENT:
		dev_info(dev, "[USB_DEBUG]CI13XXX_CONTROLLER_RESET_EVENT received\n");
		ci13xxx_msm_reset();
		break;
	case CI13XXX_CONTROLLER_DISCONNECT_EVENT:
		dev_info(dev, "[USB_DEBUG]CI13XXX_CONTROLLER_DISCONNECT_EVENT received\n");
		ci13xxx_msm_disconnect();
		ci13xxx_msm_resume();
		break;
	case CI13XXX_CONTROLLER_CONNECT_EVENT:
		dev_info(dev, "[USB_DEBUG]CI13XXX_CONTROLLER_CONNECT_EVENT received\n");
		ci13xxx_msm_connect();
		break;
	case CI13XXX_CONTROLLER_SUSPEND_EVENT:
		dev_info(dev, "[USB_DEBUG]CI13XXX_CONTROLLER_SUSPEND_EVENT received\n");
		ci13xxx_msm_suspend();
		break;
	case CI13XXX_CONTROLLER_RESUME_EVENT:
		dev_info(dev, "[USB_DEBUG]CI13XXX_CONTROLLER_RESUME_EVENT received\n");
		ci13xxx_msm_resume();
		break;
	case CI13XXX_CONTROLLER_ERROR_EVENT:
		dev_info(dev, "[USB_DEBUG]CI13XXX_CONTROLLER_ERROR_EVENT received\n");
	...
	case CI13XXX_CONTROLLER_UDC_STARTED_EVENT:
		...
		break;
	default:
		dev_info(dev, "[USB_DEBUG]unknown ci13xxx_udc event=%d\n",event);
		break;
	}
}
