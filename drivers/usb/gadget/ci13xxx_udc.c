...
static void isr_resume_handler(struct ci13xxx *udc)
{
    ...
    info("[USB_DEBUG]isr_resume_handler:speed=%d,suspended=%d \n", udc->gadget.speed,udc->suspended);
	if (udc->suspended) {
		spin_unlock(udc->lock);
		if (udc->udc_driver->notify_event)
			udc->udc_driver->notify_event(udc,
...
{
    info("[USB_DEBUG]isr_suspend_handler:speed=%d,suspended=%d,vbus_active=%d \n",
             udc->gadget.speed,udc->suspended,udc->vbus_active);
