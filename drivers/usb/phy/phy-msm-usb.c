...
extern void charge_into_sleep_mode(unsigned int value);
...
static const char *chg_to_string(enum usb_chg_type chg_type)
{
    ...
    /* <DST2015112403123 yinfenghang 20151124 begin */
#ifdef CONFIG_HUAWEI_KERNEL
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	pr_info("[USB_DEBUG]msm_otg_sm_work:%s work\n",
                  usb_otg_state_string(otg->phy->state));
...
#ifdef CONFIG_HUAWEI_KERNEL
            if (test_bit(ID_A, &motg->inputs)){
                pr_info("sm state OTG_STATE_A_WAIT_BCON: call into msm_otg_notify_charger\n");
                msm_otg_notify_charger(motg, IDEV_CHG_MIN);
            }
            else{
                pr_info("sm state OTG_STATE_A_WAIT_BCON: call into msm_hsusb_vbus_power\n");
                msm_hsusb_vbus_power(motg, 0);
            }
#else
            if (test_bit(ID_A, &motg->inputs))
...
#ifdef CONFIG_HUAWEI_KERNEL
            if (!test_bit(ID_A, &motg->inputs)){
                pr_info("sm state OTG_STATE_A_HOST: call into msm_hsusb_vbus_power\n");
                msm_hsusb_vbus_power(motg, 0);
            }
            else{
                pr_info("sm state OTG_STATE_A_HOST: NOT call into msm_hsusb_vbus_power !!!\n");
            }
#else
            if (!test_bit(ID_A, &motg->inputs))
                msm_hsusb_vbus_power(motg, 0);
...
static int msm_otg_pm_resume(struct device *dev)
{
	...
	dev_info(dev, "OTG PM resume--async_int:%d,phy_irq_pending:%d,sm_work_pending:%d\n",
		motg->async_int, motg->phy_irq_pending,motg->sm_work_pending);
