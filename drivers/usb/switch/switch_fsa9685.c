...
{
    hwlog_err("%s: usb notifier with %d\n", __func__, event);
    ...
    if (unlikely(reg_intrpt < 0)) {
        ...
        if (reg_intrpt & FSA9685_ATTACH){
            ...
            if (reg_dev_type3 & FSA9685_CUSTOMER_ACCESSORY7) {
                ...
                hwlog_info("%s: FSA9685_CUSTOMER_ACCESSORY7 ENTER\n", __func__);
            }
                          ...
                          struct device_attribute *attr, const char *buf, size_t size)
{
	...
	value = fsa9685_read_reg(FSA9685_REG_MANUAL_SW_2);
    if (value < 0){
        hwlog_err("%s: read FSA9685_REG_MANUAL_SW_2 error!!! \n", __func__);
        return -1;
    }
    switch (jig_val) {
        case JIG_PULL_DEFAULT_DOWN:
            hwlog_info("%s:pull down jig pin to default state\n", __func__);
            ret = fsa9685_write_reg(FSA9685_REG_MANUAL_SW_2, (value | REG_JIG_DEFAULT_DOWN));
            if (ret < 0) {
        ...
        case JIG_PULL_UP:
            ...
            ret = fsa9685_write_reg(FSA9685_REG_MANUAL_SW_2, (value & REG_JIG_UP));
            if (ret < 0) {
                hwlog_err("%s:write FSA9685_REG_MANUAL_SW_2 error!!!\n", __func__);
            }
            break;
