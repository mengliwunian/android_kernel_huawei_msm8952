	...
	struct device *dev, unsigned long addr, int flags, void *token)
{
	...
	if (!no_page_fault_log) {
		dsm_client_record(gpu_dclient,"kgsl iommu fault");
		dsm_client_record(gpu_dclient,"cmd:@echo 0x3 >> /sys/class/kgsl/kgsl-3d0/ft_pagefault_policy");
		dsm_client_notify(gpu_dclient, DSM_GPU_PAGE_FAULT_NO);
