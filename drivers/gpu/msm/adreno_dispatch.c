...
static int dispatcher_do_fault(struct kgsl_device *device)
{
	...
	if (cmdbatch &&
		...
		dsm_client_record(gpu_dclient,"GPU SNAPSHOT\n");
		dsm_client_record(gpu_dclient,"cmd:@rm -rf /data/hwzd_logs/dmd_log/dmd_spcial_log/*");
		dsm_client_record(gpu_dclient,"@cp /sys/class/kgsl/kgsl-3d0/snapshot/dump /data/hwzd_logs/dmd_log/dmd_spcial_log/gpu_snapshot");
		dsm_client_notify(gpu_dclient, DSM_GPU_SNAPSHOT_ERROR_NO);
	}
