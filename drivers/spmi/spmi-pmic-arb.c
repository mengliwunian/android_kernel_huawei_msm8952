...
__pmic_arb_periph_irq(int irq, void *dev_id, bool show)
{
	...
	u32 status;
	/* none irq counter */
	static u32 irq_none_cnt = 0;
	/* the counter for 100000 times */
	static u32 irq_whole_cnt = 0;
	char buf[BUF_SIZE] = {0,};
	...
	if (!acc_valid) {
		for (i = pmic_arb->min_intr_apid; i <= pmic_arb->max_intr_apid; i++) {
			if (!is_apid_valid(pmic_arb, i))
				continue;
			irq_status = readl_relaxed(pmic_arb->intr + pmic_arb->ver->irq_status(i));
			if (irq_status) {
				dev_dbg(pmic_arb->dev,
					"Dispatching for IRQ_STATUS_REG:0x%llx IRQ_STATUS:0x%x\n",
					pmic_arb->ver->irq_status(i), irq_status);
	...
	if (IRQ_NONE == ret && !show) {
		/* for none irq, we report to dmd as abnormal irq issue every 100000 times */
		if (irq_none_cnt >= 100000) {
			ad_dev_loge(pmic_arb->dev,
					"Peripheral interrupt detected. first: 0x%x, last: 0x%x, irq: 0x%x\n",
					first, last, irq);
			irq_whole_cnt++;
			snprintf(buf, BUF_SIZE, "spmi interrupt error, count %d\n", irq_whole_cnt);
			dsm_post_chg_bms_info(DSM_CHG_SPMI_INT_ERR, buf);
			irq_none_cnt = 0;
		}
		/* to avoid spmi irq issue, we return IRQ_HANDLED forcely */
		ret = IRQ_HANDLED;
		irq_none_cnt++;
	}
	if (show && irq_whole_cnt > 0) {
		ad_dev_loge(pmic_arb->dev,
				"Peripheral interrupt detected. first: 0x%x, last: 0x%x, irq: 0x%x, whole_cnt: 0x%x\n",
				first, last, irq, irq_whole_cnt);
		if (irq_whole_cnt > 100000) {
			irq_whole_cnt = 1;
        }
	}
...
static void pmic_arb_clear_spmi(struct spmi_pmic_arb_dev *pmic_arb)
{
	...
	for (apid = 0; apid < pmic_arb->max_periph_intrs; ++apid) {
		...
		if (owner == pmic_arb->ee) {
			writel_relaxed(0xff, pmic_arb->intr + pmic_arb->ver->irq_clear(apid));
			writel_relaxed(0x0, pmic_arb->intr + pmic_arb->ver->acc_enable(apid));
		}
	}
}
