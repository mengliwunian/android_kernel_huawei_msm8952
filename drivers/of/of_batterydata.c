		...
		const char *psy_name,  const char  *batt_type)
{
...
#ifdef CONFIG_HUAWEI_KERNEL
	if (!print_once)
		pr_info("FG:sense batt_id_kohm:%d, id_range_pct:%d\n", batt_id_kohm, id_range_pct);
#endif
	...
	for_each_child_of_node(batterydata_container_node, node) {
		if (batt_type != NULL) {
			...
			rc = of_batterydata_read_batt_id_kohm(node,
							...
							&batt_ids);
			if (rc) {
				pr_err("attr qcom,batt-id-kohm not exist!\n");
				continue;
			}
			...
			for (i = 0; i < batt_ids.num; i++) {
				...
				} else if ((NULL == best_node) && (NULL == default_node)) {
					default_node = node;
					best_id_kohm = batt_ids.kohm[i];
				}
			}
	...
	if (best_node == NULL) {
		if (NULL != default_node) {
			...
			pr_info("Use default battery data, batt_id_kohm:%d, best_id_kohm:%d\n", batt_id_kohm, best_id_kohm);
		} else {
			pr_err("No battery data found, batt_id_kohm:%d\n", batt_id_kohm);
		}
		return best_node;
	}
