...
int get_antenna_sub_board_voltage(void)
{
    ...
    ldo16 = regulator_get(NULL, "custom_l16");
    if (IS_ERR(ldo16)) {
        error = PTR_ERR(ldo16);
    ...
    error= regulator_set_voltage(ldo16, 1800000, 1800000);
    if (error) {
        pr_err("ldo16 regulator set voltage failed, error=%d\n", error);
        goto fail_put_ldo;
    }
    error = regulator_enable(ldo16); // enable 
    if (error) {
        pr_err("ldo16 enable failed, error=%d\n", error);
        goto fail_put_ldo;
    }
    if( qpnp_pmi_vadc == NULL)
    {
    ...
    pr_info("get_sub_board_temp %d %lld\n",
        results.adc_code, results.physical);
    error = regulator_disable(ldo16);//disable
    if (error) {
        pr_err("ldo16 disable failed, error=%d\n", error);
        goto fail_put_ldo;
    }
...
static int get_sub_board_voltage(char *buf,struct kernel_param *kp )
{
   ...
   return snprintf(buf,BUF_MAX_LENGTH, "%d", (int)voltage);
}
...
static int qpnp_vadc_probe(struct spmi_device *spmi)
{
	...
	rc = of_property_read_u32(spmi->dev.of_node,"chg_mpp_number",&chg_mpp_number);
	if(rc)
	{
		dev_err(&spmi->dev, "failed to read chg_mpp_number device tree\n");
	}
    if(0 == vadc->adc->slave)
    {
        qpnp_vadc = vadc;
