...
struct tp_lx_cal_parameter tp_module_parameter[MODULE_MANUFACTURE_NUMBER] = {{.tp_module_id = 0x55},{.tp_module_id = 0x55},{.tp_module_id = 0x55},{.tp_module_id = 0x55}};
...
static int bh1745_report_instant_power_off(struct rgb_bh1745_data* data)
{
	int i = 0;
	int j = 0;
	int size = 0;
	...
	if(dsm_client_ocuppy(bh1745_als_dclient)){
		/* buffer is busy */
		BH1745_ERR("%s: buffer is busy!, errno = %d\n", __func__,DSM_LPS_INSTANT_POWER_OFF_ERROR);
		return  -EBUSY;
	}

	for(i = 0; i < 3; i++ )
	{
		reg[i] = rgb_bh1745_i2c_read(client, (0x40+i), BH1745_I2C_BYTE);
		if(reg[i] <0){
	...
	reg[i] = rgb_bh1745_i2c_read(client, (0x44), BH1745_I2C_BYTE);

	/* read all register value and print to user*/
	for(i = i+1,j=0; j < BH1745_REG_LEN+1; j++ )
	{
		reg[i] = rgb_bh1745_i2c_read(client, (0x50+j), BH1745_I2C_BYTE);
		if(reg[i] <0){
			BH1745_ERR("%s,line %d:read %d reg failed\n",__func__,__LINE__,i);
			return reg[i];
		}
	...
	reg[i] = rgb_bh1745_i2c_read(client, (0x92), BH1745_I2C_BYTE);
	if(reg[i] < 0){
		BH1745_ERR("%s,line %d:ret = %d,rgb_bh1745_i2c_read failed\n",__func__,__LINE__,i);
		return reg[i];
	}

	/* report instant power off info */
	size = dsm_client_record(bh1745_als_dclient,
				"reg[0x40~0x55]=%d, %d, %d, %d, %d, %d, %d, %d, %d, %d\n"
				"reg[0x56~0x92]%d, %d, %d, %d, %d, %d\n"
				,reg[0x00],reg[0x01],reg[0x02],reg[0x03],reg[0x04],reg[0x05],reg[0x06],reg[0x07],reg[0x08],reg[0x09]
				,reg[0x0a],reg[0x0b],reg[0x0c],reg[0x0d],reg[0x0e],reg[0x0f]);

	dsm_client_notify(bh1745_als_dclient, DSM_LPS_INSTANT_POWER_OFF_ERROR);

	BH1745_INFO("%s,line %d,dsm error, errno = %d \n",__func__,__LINE__,DSM_LPS_INSTANT_POWER_OFF_ERROR);

	BH1745_INFO("reg[0x40~0x55]=0x%2x, 0x%2x, 0x%2x, 0x%2x, 0x%2x, 0x%2x, 0x%2x, 0x%2x, 0x%2x, 0x%2x\n"
			"reg[0x56~0x92]0x%2x, 0x%2x, 0x%2x, 0x%2x, 0x%2x, 0x%2x\n",
			reg[0x00],reg[0x01],reg[0x02],reg[0x03],reg[0x04],reg[0x05],reg[0x06],reg[0x07],reg[0x08],reg[0x09],
			reg[0x0a],reg[0x0b],reg[0x0c],reg[0x0d],reg[0x0e],reg[0x0f]);
	return size;
}
						...
						const char *buf, size_t count)
{
	...
	else if(DSM_LPS_INSTANT_POWER_OFF_ERROR == mode){
		ret = bh1745_report_instant_power_off(data);
	}
	else{
