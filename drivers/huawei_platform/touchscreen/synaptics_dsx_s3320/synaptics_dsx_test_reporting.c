...
static int synaptics_set_report_rate(unsigned char report_rate) 
{
	int rc = 0;
	...
	if(rc < 0){
		tp_log_err("%s(%d):read report rate from ic fail,base_addr=%d, offset=%d, rc=%d\n",
			__func__, __LINE__, f54->control_base_addr, F54_READ_RATE_OFFSET, rc);
		return rc;
	}

	if(report_rate_in_ic == report_rate) {
		return 0;
	} else {
...
static void synaptics_change_report_rate(void)
{
	...
	// step 1: set report rate to 120HZ
	rc = synaptics_set_report_rate((unsigned char)F54_120HI_RATE);
	if (rc) {
	...
	rc = synaptics_set_report_rate((unsigned char)F54_60LOW_RATE);
	if (rc) {
		...
		struct device_attribute *attr, char *buf)
{
	...
	strncat(g_mmi_buf_f54test_result, ";", surplus_space - 1);
	...
	if(NULL != g_mmi_buf_rx_tx_data) {
		surplus_space = str_surplus_space(buf, PAGE_SIZE);
		strncat(buf, g_mmi_buf_rx_tx_data, surplus_space - 1);
	}
...
// clear irq
static ssize_t hw_synaptics_clear_irq_show(struct kobject *dev,
		struct kobj_attribute *attr, char *buf)
{
	int retval = 0;
	...
	retval = f54->fn_ptr->read(f54->rmi4_data,
			f54->rmi4_data->f01_data_base_addr,
			data,
			f54->rmi4_data->num_of_intr_regs + 1);
	if (retval < 0) {
		tp_log_err("%s(line %d): Failed to read flash status\n",
			__func__,__LINE__);
		return retval;
	} else {
		tp_log_info("%s %d:irq_reg[0]=0x%04x, irq_reg[1]=0x%04x\n",
			__func__, __LINE__, data[0], data[1]);
	}

	// read tp reg status
	retval = f54->fn_ptr->read(f54->rmi4_data,
			f54->rmi4_data->f01_ctrl_base_addr,
			data, 2);
	if (retval < 0) {
		tp_log_err("%s(line %d): Failed to read flash status\n",
			__func__,__LINE__);
	...
	irq_gpio_status = gpio_get_value(f54->rmi4_data->board->irq_gpio);
	tp_log_info("%s(line %d): Second time read irq_gpio_status=%d\n",
			__func__,__LINE__, irq_gpio_status);

	irq_count = get_irq_count();
	tp_log_info("%s(line %d): irq_count=%ld\n",
...
static struct kobj_attribute synaptics_clear_irq = {
	.attr = {.name = "synaptics_clear_irq", .mode = 0444},
	.show = hw_synaptics_clear_irq_show,
	.store = NULL,
};
...
static int add_synaptics_mmi_test_interfaces(void)
{
	...
	tp_log_info("%s: synaptics_clear_irq start create\n", __func__);
	error = sysfs_create_file(properties_kobj, &synaptics_clear_irq.attr);
	if (error) {
		kobject_put(properties_kobj);
		tp_log_err("%s: synaptics_clear_irq create file error = %d\n", __func__,error);
		return -ENODEV;
	}
