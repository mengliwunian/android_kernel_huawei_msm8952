...
int synaptics_s3718_set_appinfo(struct synaptics_rmi4_data *rmi4_data)
{
	...
	snprintf(touch_s3718_info,sizeof(touch_s3718_info),"synaptics_dsx_%s.%x%02x", module_name, s3718_config_id[2], s3718_config_id[3]);
...
int synaptics_get_fw_data_s3718_boot(void)
{
	...
	tp_log_info("productname:%s module_name:%s\n", fwu->rmi4_data->board->product_name,module_name);
	...
	tp_log_info("Firmware get successful,Firmware image size = %ld\n", fwu->fw_entry_boot->size);
	return NO_ERR;
}
...
int synap_update_work_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	ret = synaptics_fw_data_s3718_init(rmi4_data);
	if (ret) {
		tp_log_err("%s: failed to do s3718 init, ret=%d\n", __func__, ret);
		return 0;
	}
