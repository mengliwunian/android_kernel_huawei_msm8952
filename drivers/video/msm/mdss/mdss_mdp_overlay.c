...
static void do_color_temprature(struct msmfb_mdp_pp* p_mdp_pp)
{
	...
	g = p_mdp_pp->data.pcc_cfg_data.g.g;
	...
	switch(saved_ct_data.modify_flag){
		...
		case CT_MODIFY_RED:
			...
			break;
		case CT_MODIFY_RGB:
			r = (uint32_t) (r-r*saved_ct_data.attenuation_coeff_r/1000);
			g = (uint32_t) (g-g*saved_ct_data.attenuation_coeff_g/1000);
			b = (uint32_t) (b-b*saved_ct_data.attenuation_coeff_b/1000);
			p_mdp_pp->data.pcc_cfg_data.r.r = r;
			p_mdp_pp->data.pcc_cfg_data.g.g = g;
			p_mdp_pp->data.pcc_cfg_data.b.b = b;
			/*pp_calib*.xml may send MDP_PP_OPS_DISABLE, so enable it for color temprate*/
			if( p_mdp_pp->data.pcc_cfg_data.ops & MDP_PP_OPS_DISABLE)
				p_mdp_pp->data.pcc_cfg_data.ops = MDP_PP_OPS_ENABLE | MDP_PP_OPS_WRITE;
				...
				void __user *argp)
{
	...
	case mdp_op_pre_pcc_cfg:
		...
		pr_info("2D_CT:mdp_op_pre_pcc_cfg:attenuation_coeff=%u, modify_flag=%u\n",  saved_ct_data.attenuation_coeff, saved_ct_data.modify_flag);
		pr_info("3D_CT:mdp_op_pre_pcc_cfg:attenuation_coeff_r=%u,attenuation_coeff_g=%u,attenuation_coeff_b=%u, modify_flag=%u\n",  
								saved_ct_data.attenuation_coeff_r,saved_ct_data.attenuation_coeff_g,saved_ct_data.attenuation_coeff_b, saved_ct_data.modify_flag);
...
int mdss_mdp_overlay_init(struct msm_fb_data_type *mfd)
{
	...
	saved_ct_data.attenuation_coeff_r = 0;
	saved_ct_data.attenuation_coeff_g = 0;
	saved_ct_data.attenuation_coeff_b = 0;
