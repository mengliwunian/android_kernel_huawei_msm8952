...
static const char  *lcdtype[] = {"BOE_OTM1906C_5P2_1080PXA_CMD",
								"EBBG_OTM1906C_5P2_1080PXA_CMD",
								"INX_OTM1906C_5P2_1080PXA_CMD"};
				...
				if(enable_PT_test)
					{
						if(!strcmp(in_vreg[i].vreg_name, "ibb") || !strcmp(in_vreg[i].vreg_name, "lab"))
							{
								DEV_INFO("enter PT test,enable VSP/VSN power\n");
								continue;
							}
					}
						...
						if(!strcmp(in_vreg[i].vreg_name, "ibb") || !strcmp(in_vreg[i].vreg_name, "lab"))
							{
								if(in_vreg[i].vreg->rdev->use_count > 1)
									{
										DEV_INFO("Exit PT test,disable VSP/VSN power\n");
