...
#ifdef CONFIG_HUAWEI_LCD
u32 mdss_dsi_panel_cmd_read_gen(struct mdss_dsi_ctrl_pdata *ctrl, char cmd0,
		char cmd1, void (*fxn)(int), char *rbuf, int len)
{
	struct dcs_cmd_req cmdreq1;
	struct mdss_panel_info *pinfo;
	struct dsi_panel_cmds *pcmds;
	static char dcs_cmd1[2] = {0x00, 0x00}; /* DTYPE_GEN_READ1 */
	static struct dsi_cmd_desc dcs_read_cmd1 = {
		{DTYPE_GEN_READ1, 1, 0, 1, 5, sizeof(dcs_cmd1)},
		dcs_cmd1
	};
	...
	cmdreq1.cmds = &dcs_read_cmd1;
	cmdreq1.cmds_cnt = 1;
	cmdreq1.flags = CMD_REQ_RX | CMD_REQ_COMMIT;
	if(ctrl->esd_check_enable)
	{
		pcmds = &ctrl->esd_cmds;
		if (pcmds->link_state == DSI_HS_MODE)
	...
	cmdreq1.rlen = len;
	cmdreq1.rbuf = rbuf;
	cmdreq1.cb = fxn; /* call back */
	mdss_dsi_cmdlist_put(ctrl, &cmdreq1);

	return 0;
}
/***************************************************************
Function: mdss_dsi_panel_cmd_read_local
Description: when read multi regs to void return error value
 Parameters:same as mdss_dsi_panel_cmd_read
Return:0:success
Return:-EINVAL:fail
***************************************************************/

u32 mdss_dsi_panel_cmd_read_local(struct mdss_dsi_ctrl_pdata *ctrl, char cmd0,
		char cmd1, void (*fxn)(int), char *rbuf, int len)
{
	struct dcs_cmd_req cmdreq;
	struct mdss_panel_info *pinfo;
...
static int mdss_dsi_check_panel_status(struct mdss_panel_data *pdata)
{
	...
	else
	{
		...
		do{
			if (ctrl_pdata->reg_read_local)
			{
				mdss_dsi_panel_cmd_read_local(ctrl_pdata,0x0A,0x00,NULL,rdata,read_length);
...
int panel_check_live_status(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	for(j = 0;j < ctrl->esd_cmds.cmd_cnt;j++)
	{
		...
		{
			compare_reg = 0;
			if (ctrl->reg_read_gen)
			{
				mdss_dsi_panel_cmd_read_gen(ctrl,ctrl->esd_cmds.cmds[j].payload[0],0x00,NULL,esd_buf,ctrl->esd_cmds.cmds[j].dchdr.dlen - 1);
				LCD_LOG_DBG("%s: Esd read cmd type is DTYPE_GEN_READ1\n", __func__);
			}
			else if (ctrl->reg_read_local)
			{
				mdss_dsi_panel_cmd_read_local(ctrl,ctrl->esd_cmds.cmds[j].payload[0],0x00,NULL,esd_buf,ctrl->esd_cmds.cmds[j].dchdr.dlen - 1);
				LCD_LOG_DBG("%s: The count of esd reg read is more than one\n", __func__);
			}
			else
			{
				mdss_dsi_panel_cmd_read(ctrl,ctrl->esd_cmds.cmds[j].payload[0],0x00,NULL,esd_buf,ctrl->esd_cmds.cmds[j].dchdr.dlen - 1);
				LCD_LOG_DBG("%s: The initial reg read interface function\n", __func__);
			}
			count++;
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	rc = of_property_read_u32(np, "huawei,reg-read-gen-flag", &tmp);
	ctrl_pdata->reg_read_gen = (!rc ? tmp : 0);
	rc = of_property_read_u32(np, "huawei,reg-read-local-flag", &tmp);
	ctrl_pdata->reg_read_local = (!rc ? tmp : 0);
